...
function setupVideoArchiveToggle() {
    const archiveToggle = document.getElementById('video-archive-toggle');
    const archivePanel = document.getElementById('video-archive-panel');
    const archiveHeading = document.getElementById('video-archive');

    if (!archiveToggle || !archivePanel || !archiveHeading) return;

    const revealArchive = () => {
        archivePanel.hidden = false;
        archiveToggle.setAttribute('aria-expanded', 'true');
        archiveHeading.classList.add('visible');

        const archiveGrid = archivePanel.querySelector('.masonry-gallery');
        if (archiveGrid) {
            archiveGrid.classList.add('visible');
        }
    };
    ...
    if (window.location.hash === '#video-archive') {
        revealArchive();
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 6. Reveal the video archive only after clicking "More Videos"
    setupVideoArchiveToggle();
});
